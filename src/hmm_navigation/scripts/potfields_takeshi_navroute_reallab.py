...
def readSensor(data):
    ...
    #     
        ...
        if (xythcuant==xyth_nxt_cuant or  euclD <=0.5):
     ...
     Ftotx= Fmag*np.cos(Fth)*.00251   +  2.5*  Fmagat*np.cos(Fatrth)
     
     Ftoty= Fmag*np.sin(Fth)*.00251    + 2.5*   Fmagat*np.sin(Fatrth)
     
     Ftotth=np.arctan2(Ftoty,Ftotx)
     print ('Ftotx,Ftoty,Ftotth',Ftotx,Ftoty,Ftotth)
     ...
     """
                    if( abs(Ftotth) < .2) :#or (np.linalg.norm((Fx,Fy)) < 100):
                       ...
                       if Ftotth < -.4:
                           ...
                           if Ftotth < -1.57:
                               print('ang--')
                               speed.linear.x=-0.003
                               speed.angular.z=-0.7 
                       
               
               
                       if Ftotth > 0.2:
                           speed.linear.x=0.008
                           speed.angular.z=1
...
def inoutinout():
    ...
    pose  = rospy.Subscriber('/global_pose',PoseStamped, newOdom)
